...
<div class="breaking-changes">
	...
	<h3 style="margin-top: 0;">
		<?php _e('Breaking Changes', 'vikrentcar'); ?>
	</h3>
		...
		<ul style="padding-left: 10px;">
			...
			foreach ($list as $file)
			{
				...
				<li style="display: flex; align-items: center;">
					...
					<a
						href="admin.php?page=vikrentcar&view=overrides&client=<?php echo $client; ?>&overridefile=<?php echo base64_encode($file); ?>"
						target="_blank"
						style="margin: 0;"
					><i class="<?php echo VikRentCarIcons::i('external-link-square'); ?>" style="font-size: 18px;"></i></a>
				</li>
				<?php
			}
			?>
	...
	<button class="button" id="breaking-changes-dismiss"><?php _e('Dismiss', 'vikrentcar'); ?></button>
...
<script>
	(function($) {
		...
		const unregisterBreakingChanges = (files) => {
			doAjax(
				'admin-ajax.php?action=vikrentcar&task=override.dismissbc',
				{
		...
		$(function($) {
			...
			$('#breaking-changes-copy').on('click', function() {
				...
				VRCCore.copyToClipboard($(this).prev('textarea')).then(() => {
