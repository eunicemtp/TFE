...
/**
 * VikRentCar plugin Overrides model.
 * @wponly
 *
 * @since 1.4.0
 * @see   JModel
 */
class VikRentCarModelOverrides extends JModel
{
	/**
	...
	protected function getViewsTree(string $client)
	{
		...
		{
			// admin path
			$path = VRC_ADMIN_PATH . DIRECTORY_SEPARATOR . 'views';
		...
		{
			// site path
			$path = VRC_SITE_PATH . DIRECTORY_SEPARATOR . 'views';
		...
		$overridesPath = JPath::clean($upload['basedir'] . '/vikrentcar/overrides/' . $clientFolder . '/');
		...
		if (JFolder::exists(VIKRENTCAR_LIBRARIES . '/mvc/' . $clientFolder . '/views'))
		{
			$views = array_merge($views, JFolder::folders(VIKRENTCAR_LIBRARIES . '/mvc/' . $clientFolder . '/views', '.', $recursive = false, $fullPath = true));
		}
	...
	protected function getLayoutsTree()
	{
		...
		$tree[] = [
			'name'   => 'admin',
			'path'   => VRC_ADMIN_PATH . DIRECTORY_SEPARATOR . 'layouts',
			'folder' => true,
			'files'  => [],
		];
		...
		// $tree[] = [
		// 	'name'   => 'site',
		// 	'path'   => VRC_SITE_PATH . DIRECTORY_SEPARATOR . 'layouts',
		// 	'folder' => true,
		// 	'files'  => [],
		// ];
		...
		$tree[] = [
			'name'   => 'core',
			'path'   => VIKRENTCAR_LIBRARIES . DIRECTORY_SEPARATOR . 'html',
			'folder' => true,
			'files'  => [],
		];
		...
		// create base path of overrides
		$overridesPath = JPath::clean($upload['basedir'] . '/vikrentcar/layouts/');
	...
	protected function getModulesTree()
	{
		...
		// create base path of overrides
		$overridesPath = JPath::clean($upload['basedir'] . '/vikrentcar/overrides/modules/');

		// get all folders at the specified path
		$modules = JFolder::folders(VIKRENTCAR_BASE . '/modules', '.', $recursive = false, $fullPath = true);
		...
		foreach ($modules as $module)
		{
			...
			if (!$this->isExcluded($modName, $this->_excludedModules))
			{
				// create node to inject within the tree
				$node = [
					'name'   => preg_replace("/^mod_vikrentcar_/i", '', $modName),
					'path'   => $module,
					'folder' => true,
					'files'  => [],
				];
	 ...
	 * In example, default_foo_bar.php is not part of VikRentCar and therefore it
	 * will be discarded.
	 * 
	 * @return  array
	 */
	public function getAllOverrides()
	{
		...
		$lookup = [
			'admin'   => JFolder::files($upload['basedir'] . '/vikrentcar/overrides/admin/', '\.php$', true, true),
			'site'    => JFolder::files($upload['basedir'] . '/vikrentcar/overrides/site/', '\.php$', true, true),
			'layouts' => JFolder::files($upload['basedir'] . '/vikrentcar/layouts/', '\.php$', true, true),
			'modules' => JFolder::files($upload['basedir'] . '/vikrentcar/overrides/modules/', '\.php$', true, true),
		];
